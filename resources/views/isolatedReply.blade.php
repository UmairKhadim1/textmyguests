...
<head>
  ...
  <meta property="og:title" content="{{ $reply['sender'] }}'s {{ intval($mediaCount) > 1 ? 'pictures' : ($media ? 'picture' : 'message') }} from {{ $shindig->name }} | TextMyGuests" />
  <meta property="og:image" content={{ $media ?? env('APP_URL') . "/assets/images/live/TMG_og_image.jpg" }} />
  <meta property="og:description" content="Check out the entire live stream of pictures and messages from {{ $shindig->name }} on TextMyGuests." />
  ...
  <title>{{ $reply['sender'] }}'s {{ intval($mediaCount) > 1 ? 'pictures' : ($media ? 'picture' : 'message') }} | TextMyGuests</title>
